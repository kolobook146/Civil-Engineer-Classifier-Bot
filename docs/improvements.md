...
| IMP-038 | Add an FTS-first search layer over the published project schedule | P2 | post-pilot | planned | TBD | Reason: most schedule queries are structured lookups by task name, function, stage, work type, responsible party, date, or task ID, so full-text search should be the first retrieval path before any semantic/vector search. Result: faster, cheaper, and more predictable schedule search in Telegram. |
| IMP-039 | Add Telegram dashboard preview export from `dashboard_visual` | P2 | pilot | done | TBD | Implemented via `src/infrastructure/dashboard_sheets_exporter.py`, `src/presentation/telegram_polling_handler.py`, and `src/presentation/notification_service.py`: the bot now exposes an on-demand `Dashboard` menu action, exports `dashboard_visual!A1:X38` through the official Google Sheets PDF path, archives both `PDF` and `JPEG` in `var/dashboard_exports`, converts via macOS `sips`, and sends the JPEG back to Telegram as a photo. Result: users can receive the latest visual dashboard in chat while the bot keeps a local runtime history of generated exports. |
...
| IMP-032 | Pre-production readiness checklist | P0 | post-pilot | planned | TBD | Consolidate go-live operations: re-enable Gemini startup preflight (`LLM_PREFLIGHT_ENABLED=true`), run full startup preflight for both processes, run integration smoke tests (Telegram + Gemini + Sheets), verify queue/worker health, rotate secrets, and confirm logging/alerting baselines. |
| IMP-033 | Show confidence in confirmation cards (`Low`/`Medium`/`High`) | P2 | post-pilot | planned | TBD | UX improvement to help users quickly assess extraction reliability before confirmation. |
| IMP-034 | Add soft warning for low confidence (`Please double-check fields.`) | P2 | post-pilot | planned | TBD | Reduce false confirmations in ambiguous parses without blocking the flow. |
| IMP-035 | Introduce a unified visual card style (single rhythm, density, and spacing) | P3 | post-pilot | planned | TBD | Improve readability and professional consistency across confirmation/success/error cards. |
| IMP-036 | Extend stage dictionary with metadata (`stage_code`, `label`, `stage_type`, `default_order`, `can_repeat`, `optional`) | P1 | post-pilot | planned | TBD | Reason: plain stage labels are not enough for robust dependency logic and stable analytics. Result: explicit stage semantics for routing, validation, and schedule recalculation. |
| IMP-037 | Introduce a function-stage dependency matrix (`allowed_stages`, `from_stage -> to_stage`) | P1 | post-pilot | planned | TBD | Reason: stage sequence and branching rules differ by function and should not be hardcoded in one flat list. Result: controllable graph construction and predictable schedule recalculation. |
