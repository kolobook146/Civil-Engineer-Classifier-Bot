...
| IMP-002 | Queue retry model (`attempt_count`, `next_attempt_at`, jitter) | P1 | post-pilot | planned | TBD | Reason: queue now stores only `status` and `last_error` (`src/infrastructure/queue_repository.py`). Result: controlled retries instead of repeatedly hitting the same task. |
...
| IMP-014 | Protect queue worker from hot-loop on LLM timeouts | P0 | pilot | planned | TBD | Reason: on `LLMTimeoutError` task is requeued immediately and loop continues without pause (`src/application/queue_worker.py`). Result: no API spam and lower load spikes. |
...
| IMP-016 | Make Gemini call deterministic (`temperature=0`, response schema/tool mode) | P1 | post-pilot | planned | TBD | Reason: current call sets only `response_mime_type` (`src/infrastructure/gemini_client.py`). Result: fewer unstable outputs for identical inputs. |
...
| IMP-018 | Sanitize/limit fallback `comment` before writing to Sheets | P1 | pilot | planned | TBD | Reason: raw LLM output can be large/noisy in `comment`. Result: lower write-failure risk and cleaner reporting data. |
| IMP-019 | Cache dictionaries and dictionary version (mtime/hash strategy) | P1 | post-pilot | planned | TBD | Reason: dictionaries are reloaded and version is recalculated per message (`src/application/classification_orchestrator.py`, `src/infrastructure/dictionary_repository.py`). Result: lower latency and I/O. |
...
| IMP-024 | Add startup preflight checks (LLM, Sheets, dictionaries, worksheet) | P1 | pilot | planned | TBD | Reason: several failures are currently detected only during message processing. Result: fail-fast startup and fewer runtime user-facing errors. |
...
| IMP-028 | Add Telegram button-based menu for main user actions | P2 | pilot | planned | TBD | Reason: current interaction relies mainly on commands and free-form text. Result: clearer UX, lower user friction, and fewer navigation mistakes. |
...
1. `IMP-014` (queue hot-loop protection on timeout).
2. `IMP-002` (retry model with backoff/jitter).
3. `IMP-021` (Google Sheets write-path optimization).
