...
| IMP-016 | Ask the user for confirmation before writing data to Google Sheets | P1 | pilot | planned | TBD | Reason: the current flow persists classification immediately after parsing, even when inferred fields may be ambiguous. Result: the user can confirm, cancel, or correct the record before persistence, reducing false positives and accidental writes. |
...
1. `IMP-021` (Google Sheets write-path optimization).
2. `IMP-016` (user confirmation before Google Sheets write).
3. `IMP-017` (preserve valid fields in fallback).
